...
PATH = ROOT / "data/generated-stegpay-authority-boundary.json"
...
def fail(message: str) -> None:
    raise SystemExit(f"GENERATED STEGPAY GUARDIAN BOUNDARY: FAIL: {message}")
...
def main() -> int:
    ...
    checks = {
        ...
        "state": value.get("state") == "BOUNDARY_EVIDENCE_IMPORTED",
        "test only": value.get("test_only") is True,
        "signature boundary": value.get("signature_boundary_preserved") is True,
        "replay safe": value.get("replay_safe_consumer_observed") is True,
    }
    false_fields = [
        "guardian_enforcement_authorized",
    ...
    if failed:
        fail(", ".join(failed))
    print("GENERATED_STEGPAY_GUARDIAN_IMPORT=PASS")
    return 0
